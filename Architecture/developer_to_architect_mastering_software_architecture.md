...
3. Primary impacts
	1. Motivations
		...
		2. Frequent Deployment - External (users can see)
			1. Incremental features can be delivered to customer at a higher frequency
				1. Positive impact on the business
		3. Internal benefits
			1. Developing independent services (for scalability)
			2. Developing independently - different teams can work on different services
				1. Helps in making a large scale system
			3. Independent deployment - for large scale systems
				1. We can release system if it is partially ready
					1. Possible if we can deploy partial system (few independent services)
					2. Consequence - enables frequent deployment of services
						1. If a service doesn't need to wait for other services to be deployed (independent deployability), then we can deploy the service whenever we want
							1. We can also deploy any number of times we want independent of the number of deployments of other services
				2. Consequence: Independent scalability
		4. Independent Services -leads to-> Independent Development -leads to-> Independent Deployment -leads to-> Frequent Deployment
	2. Summary of Motivation for Micro-Services:
		1. High Scalability
		2. Frequent Deployment

### Service Oriented Architecture ###
1. Independent
	1. Each service can have its own technology stack, libraries, frameworks, etc...
		1. Different services - They can work independently to a large extent
			1. Order service
			2. Inventory service
			3. Shipment service
		2. Example: C++ app with SOAP interface, Java app with REST APIs, ...
	2. Each service can be scaled independently and differently
		1. If there is more load on a service - we can have more instances for the service (order service say)
			1. Scaling independently is possible
2. Not Independent
	1. Common interface schema
		1. XML Schema
			1. Example: Order service & Shipment service
				1. SOA mandates that there is a common definition of Order (XML file that represents an Order that is passed to different services must be the same - data structure is exactly the same)
					1. Common schema
						1. Problem: If we add new fields, rename fields, ... (disruptive changes) in one service but keeping the schema the same in other services, it is not possible
							1. The services are tightly coupled due to interface schema (SOAP say)
				2. The services usually use a common schema (SOAP is usually used - it helps)
	2. Common database schema
		1. RDBMS Schema
			1. The services access the same database
				1. If services are accessing the schemas (multiple usually), there is a dependency
					1. order schema, inventory schema, shipment schema
						1. If shipment service is accessing order schema & shipment schema (dependency)
			2. Problems:
				1. Forces us to use the same instance of database for all services
					1. Physical machine is the same
						1. Primary RDBMS instance
							1. Entire load is on single machine
				2. If Order schema is changed say, Shipment service is using Order schema so it needs to be updated
	3. Summary:
		1. Whenever we make changes to interface schema, then we have to make changes to all dependent services
		2. Whenever we make changes to database schema, then we have to make changes to all dependent services
3. Issues
	1. Service development may be independent but not deployment
		1. Certain changes can be independent but if we do the above two kinds of changes, we will not be able to deploy services independently
			1. **We may have to make changes to all the services, test them together and deploy/release them together**
	2. Single database has scalability limitations
		1. **If all services access same database, it can only be scaled vertically (not horizontally)**

### Micro-Services Architecture Style ###
1. Shared Nothing Architecture (nothing is shared between services - no frontend interface schema, no backend database schema)
	1. Services developed and deployed independently
		1. No dependencies between services
	2. Achieved through vertical partitioning
		1. We need to select independent domains
			1. Define domains that exist in the application
			2. The domains are completely independent problems which can have completely independent systems to do the job
2. Vertical/Domain Partitioning
	1. Independent schema/database
		1. Example: Order Service has Order Database and Schema
			1. If Order service needs Shipment data, it must make a request to Shipment service
				1. Order schema can be in **separate DB instance** (**no need for any distributed database**)
					1. Order schema definition in Order service is different from Order schema definition in Shipment service
						1. Names of attributes
						2. Number of attributes
						3. Structure
	2. Loosely coupled service interfaces
		1. REST interfaces instead of XML/WSDL schemas (SOAP)
			1. Each micro-service can have independent definitions of its' data-structures
			2. How does it work?
				1. If order is placed, a transformation is done (by an orchestrator) to order in shipment service
					1. Aggregator service can do the translation
						1. Aggregator service (acts as orchestrator)
							1. It sends request to Order service
							2. Order service places order and returns response to aggregator service
							3. Aggregator service transforms the response to Shipment service Order
							4. Aggregator service sends the translated request to Shipment service
						2. **Front-end schemas get completely decoupled** (adapter pattern)
					2. Loose coupling: REST interface by itself doesn't cause loose coupling because it does not mandate a common schema
						1. **Needs writing code to transform data from one schema to another schema**
							1. **SOAP only aggregates the problem but not the source of the problem**
							2. **REST helps in creation of loosely coupled services**
	3. No reusable libraries except utilities
		1. No common libraries between services
			1. If we make some changes to the library due to one service, all dependent services need to be changed and re-deployed
			2. They are exposed to business logic
		2. Utilities:
			1. Shared utilities is fine because they don't change due to update in the business logic
			2. They do not change much
	4. Share nothing architecture gives rise to total independence => results in very high scalability
		1. We don't use microservices in all cases due to certain issues
			1. Small applications (we pay a price in terms of transactions)
3. Issues
	1. Duplicate Codebase
		1. No library re-use
		2. No DB schema re-use
	2. Transaction failures
		1. When there is one
	3. Transaction rollbacks
		1. When we need to rollback a failed transaction

### Transactions in Micro-Services ###
1. Transaction involves multiple machines (how to do this with micro-services)
	1. Distributed services with their own DB
		1. Example: Order booking
			1. Order booking request goes to Order service
			2. Order sends requests to Inventory service & Shpment service
			3. If enough material is available in the inventory
			4. Inventory service updates stock in Inventory DB
			5. If shipment can be delivered to the address or not
			6. Shipment service then persists shipment order in DB
			7. Order service confirms order in Order DB
			8. Order response is returned back to client
		2. One Global Book Order transaction is composed of 3 local transactions
			1. Local transaction: Service doing transaction on its own DB (ACID transaction)
	2. Local transaction not possible (Global transaction)
2. Options: (How to implement transactions with microservices?)
	1. Distributed ACID transactions (there is complexity, they don't provide availability & scalability so we are forced to used compensating transactions)
		1. 2PC/3PC
			1. 2 Phase Commit - transaction happens in 2 phases
				1. 1st phase - request comes to Order service (coordinator - a separate service can also be used as coordinator)
					1. Requests are sent to Inventory service & Shipment service to check if they are available to participate in the transaction
					2. Inventory service & Shipment service take locks on their respective DBs
					3. Both services will send promise to Coordinating service (Order service say)
					4. Order service takes locks on its own DBs
				2. 2nd phase - Coordinating service sends requests to commit to all participant services & then it will do its commit
					1. Response is sent to client that Order is booked
				3. If certain services fail transactions:
					1. Rollback requests are sent to other services and rejection response is sent to customer
					2. Locks are released
				4. Problem: locks are taken for long duration
					1. Transactions take time
					2. Transactions block other requests for long time
						1. Don't scale very well (not for high scalability requirements - microservices say)
			2. 3PC - improvement over 2PC
				1. Offers more reliability
					1. If coordinator goes down in between transaction
						1. Problem: Transactions get blocked
						2. Solution: elects another coordinator
				2. It has same scalability problems as 2PC
		2. Completely ACID
	2. Compensation transactions
		1. SAGA pattern (more popular with microservices - relatively new)
		2. Eventually consistent model (SAGA relaxes consistency and isolation)
			1. Relaxes consistency and isolation

### Compensating Tansactions - SAGA Pattern ###
1. 'Logical Undo' of a partially committed transactions
	1. Flow of reversal may not be exactly opposite, and some steps can be executed in parallel
	2. Compensation itself can fail. Should be able to restart itself and retry
		1. Example: Undo in inventory failed
			1. Solution: It must resume from here and retry
2. Asynchronous processing for reliability
3. Example:
	1. Order service sends request to inventory service
	2. Inventory service reserves inventory (local transaction)
		1. Doing transaction in parts with each local transaction being atomic
	3. Order service sends request to shipment service
	4. Shipment services makes reservations
	5. Order is fulfilled and order service sends response to clients
		1. 3 transactions (inventory, order, shipment)
4. Example: If something goes wrong
	1. Support inventory local transaction is successful (no locks)
	2. Shipments cannot be created and returns to order service with a failure
		1. System is in dirty state
			1. Changes are made in databases
	3. Solution: Compensating transactions
		1. Reverse the effects of previous local transactions
			1. For each transaction, we write corresponding transaction that can undo the action
				1. We can run undos in reverse order (they can run in parallel or in different sequence)
		2. Example:
			1. Undo shipment - nothing to be done
			2. Undo inventory
			3. Undo order
	4. We need to write undo logic
		1. We can carry forward or move backwards (context dependent)
			1. If services are down, we must wait and resume
5. ACID properties:
	1. Durability is guaranteed
	2. Atomicity: Overall transaction is not atomic (partially comitted transactions)
		1. End effect: Entire transaction is comitted or compensate for it
			1. Gives the effect of atomicity
	3. Consistency: In the end, the whole system can be consistent
		1. In between, system is in-consistent
		2. It is eventually consistent
	4. Isolation: Transactions are not completely isolated
		1. Not atomic
			1. Order db and inventory db have changed and simultaniously other orders and inventory changes take place
				1. The orders can interfere with each other (they see partially committed changes)
	5. Overall:
		1. Atomic, eventually consistent, not isolated, durable
